 ...
 * @Flow\Entity
 */
class Profil_Original  extends \TYPO3\Party\Domain\Model\AbstractParty {
 ...
 */
class Profil extends Profil_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface, \TYPO3\Flow\Persistence\Aspect\PersistenceMagicInterface {
	...
	public function __construct() {
		...
		parent::__construct();
				...
				try {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Polabaer\Music\Domain\Model\Profil', '__construct', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Polabaer\Music\Domain\Model\Profil', '__construct', $joinPoint->getMethodArguments());
	 ...
	 public function __clone() {
		...
		$result = NULL;
			...
			try {
					...
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Polabaer\Music\Domain\Model\Profil', '__clone', $methodArguments);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}

					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Polabaer\Music\Domain\Model\Profil', '__clone', $joinPoint->getMethodArguments());
					$result = $this->Flow_Aop_Proxy_invokeJoinPoint($joinPoint);

					$advices = $this->Flow_Aop_Proxy_targetMethodsAndGroupedAdvices['__clone']['TYPO3\Flow\Aop\Advice\AfterReturningAdvice'];
					$joinPoint = new \TYPO3\Flow\Aop\JoinPoint($this, 'Polabaer\Music\Domain\Model\Profil', '__clone', $joinPoint->getMethodArguments(), NULL, $result);
					foreach ($advices as $advice) {
						$advice->invoke($joinPoint);
					}
	...
	$reflectedClass = new \ReflectionClass('Polabaer\Music\Domain\Model\Profil');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('Polabaer\Music\Domain\Model\Profil', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('Polabaer\Music\Domain\Model\Profil', $propertyName, 'var');
