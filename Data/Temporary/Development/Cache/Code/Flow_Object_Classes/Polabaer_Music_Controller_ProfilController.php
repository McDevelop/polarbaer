...
class ProfilController_Original extends \TYPO3\Flow\Mvc\Controller\ActionController {
 ...
 * 
 */
class ProfilController extends ProfilController_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		if ('Polabaer\Music\Controller\ProfilController' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('Polabaer\Music\Controller\ProfilController');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('Polabaer\Music\Controller\ProfilController', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('Polabaer\Music\Controller\ProfilController', $propertyName, 'var');
	 ...
	 */
	 private function Flow_Proxy_injectProperties() {
		$this->injectSettings(\TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('TYPO3\Flow\Configuration\ConfigurationManager')->getConfiguration(\TYPO3\Flow\Configuration\ConfigurationManager::CONFIGURATION_TYPE_SETTINGS, 'Polabaer.Music'));
		$profilRepository_reference = &$this->profilRepository;
		$this->profilRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getInstance('Polabaer\Music\Domain\Repository\ProfilRepository');
		if ($this->profilRepository === NULL) {
			$this->profilRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getLazyDependencyByHash('143a82c66dc562422d4d0322aebc6063', $profilRepository_reference);
			if ($this->profilRepository === NULL) {
				$this->profilRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->createLazyDependency('143a82c66dc562422d4d0322aebc6063',  $profilRepository_reference, 'Polabaer\Music\Domain\Repository\ProfilRepository', function() { return \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('Polabaer\Music\Domain\Repository\ProfilRepository'); });
			}
		}
